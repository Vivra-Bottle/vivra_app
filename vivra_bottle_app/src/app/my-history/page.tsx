 ...
 export default function MyHistory() {
   ...
   const getDayConsumptionData = async (date: string) => {
     try {
       ...
       averageConsumptionval = averageConsumptionval + data.consumption;
   ...
   useEffect(() => {
     const fetchData = async () => {
       try {
         ...
         const scalingFactor = 1 + A * Math.exp(-Math.pow(((averageConductivity/7) - C_opt) / sigma, 2)) - B * Math.max(0, Math.abs((averageConductivity/7) - C_opt) - 300);
         const amountConsumed = (averageConsumptionval/7*33.814)       
         const targetConsumation = (weight/2)
         const score = Math.round((amountConsumed/targetConsumation)*scalingFactor*100);
